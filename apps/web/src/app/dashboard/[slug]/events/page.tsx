...
function EventsPanel({
  ...
  const setRegistration = trpc.events.setRequiresRegistration.useMutation({
    onSuccess: async () => {
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-ink-600 dark:text-ink-300">
          ...
          {planningCenterLinked
            ? ' Event content is synced from Planning Center; you can still require app registration.'
            : null}
        </p>
      </div>

      {planningCenterLinked ? null : (
        <Card className="border-ink-200 dark:border-ink-800 dark:bg-ink-900">
          <CardContent className="space-y-4 px-5 py-5">
            ...
            <label className="flex items-center gap-2 text-sm text-ink-700 dark:text-ink-200">
              ...
              />
              Require registration in the app
            </label>
            <Button
          ...
          <Card key={ev.id} className="border-ink-200 dark:border-ink-800 dark:bg-ink-900">
            <CardContent className="flex flex-wrap items-start justify-between gap-3 px-5 py-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  {ev.requiresRegistration ? 'Make open' : 'Require registration'}
                </Button>
                {planningCenterLinked || ev.source === 'PLANNING_CENTER' ? null : (
                  <Button
                    variant="destructive"
                    size="sm"
                    onClick={() => {
  ...
  return (
    <ChurchDashboardProvider slug={slug}>
      {({ churchId, planningCenterLinked }) => (
        <EventsPanel churchId={churchId} planningCenterLinked={planningCenterLinked} />
      )}
