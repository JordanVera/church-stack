...
async function safeDeleteBlob(url: string | null | undefined) {
  ...
  try {
    ...
    console.error('[church/pastor-photo] failed to delete previous blob', err);
  }
}
...
function errorResponse(err: unknown) {
  ...
  console.error('[church/pastor-photo]', err);
...
export async function POST(req: Request) {
  try {
    ...
    const pastorId = String(form.get('pastorId') ?? '').trim();
    ...
    if (!pastorId) {
      return NextResponse.json({ ok: false, error: 'pastorId is required' }, { status: 400 });
    }
    if (!(file instanceof File)) {
      return NextResponse.json({ ok: false, error: 'file is required' }, { status: 400 });
    }
    if (!ALLOWED_TYPES.has(file.type)) {
      return NextResponse.json({ ok: false, error: 'Use PNG, JPEG, or WebP.' }, { status: 400 });
    }
    if (file.size <= 0 || file.size > MAX_BYTES) {
      return NextResponse.json(
        { ok: false, error: 'Photo must be under 2MB.' },
        { status: 400 }
      );
    }
    ...
    const pastor = await prisma.pastor.findFirst({
      where: { id: pastorId, churchId: church.id },
      select: { id: true, photoUrl: true },
    });
    if (!pastor) {
      return NextResponse.json({ ok: false, error: 'Pastor not found' }, { status: 404 });
    }

    const previousUrl = pastor.photoUrl;
    const pathname = `churches/${church.slug}/pastors/${pastor.id}-${crypto.randomUUID()}.${extForMime(file.type)}`;
    ...
    return NextResponse.json({ ok: true, photoUrl: updated.photoUrl });
  } catch (err) {
...
export async function DELETE(req: Request) {
  try {
    ...
    const pastorId = body?.pastorId?.trim();
    if (!churchId) {
      return NextResponse.json({ ok: false, error: 'churchId is required' }, { status: 400 });
    }
    if (!pastorId) {
